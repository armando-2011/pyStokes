...
Created by Andrés González Mancera on 2010-02-25.
Copyright (c) 2010 Universidad de los Andes. All rights reserved.
"""
...
def triang(shape = 'ico', maxlevel = 4, r = 1.0 ,winding = 0):
    """
...
def mesh_refine_tri4(FV):
    ...
    for f in xrange(Nface):
        ...
        # TODO Aquí vamos, seguir conversion
        F2[(f+1)*4-4,:] = [ NA, Na, Nc ]
        F2[(f+1)*4-3,:] = [ Na, NB, Nb ]
        F2[(f+1)*4-2,:] = [ Nc, Nb, NC ]
        F2[(f+1)*4-1,:] = [ Na, Nb, Nc ]
...
def cell_project(v,r):
    ...
    for i in xrange(v.shape[0]):
        ...
        a = ((X**2.0)+(Y**2.0))/(R**2.0)
        b = (c0 + c1*a + c2*(a**2.0))
        Z = (0.5*R*(np.abs(1.0-a)**0.5))*b
        vecnorm = v[i]/norm(v[i])
        v[i][0]=X
        v[i][1]=Y
        v[i][2]=Z*np.sign(vecnorm[2])
        
def save_xml(ug, file_name):
    """Save the unstructured grid dataset to a VTK XML file."""
    w = tvtk.XMLUnstructuredGridWriter(input=ug, file_name=file_name, data_mode = 0)
    w.write()
...
def main():
    ...
    tension = tvtk.FloatArray(name='tension')
    tau = np.zeros(FV.vertices.shape[0],dtype=float)
    tension.extend(tau)
    ug.point_data.add_array(tension)
    save_xml(ug, 'mesh_ref3.vtu')
